...
<div class="dashboard-content">
    ...
    <div class="row dashboard-section">
        ...
        @include('components.card-statistic', ['label' => 'Upgrade Pending', 'value' => $pendingUpgrades, 'icon' => 'arrow-up-circle', 'variant' => 'danger', 'description' => $pendingUpgrades === 0 ? 'Tidak ada upgrade pending' : 'Perlu keputusan admin'])
    </div>
    ...
    <div class="row dashboard-panel-row dashboard-section">
        ...
        <div class="col-12 col-lg-6"><div class="card h-100"><div class="card-header"><h4>Upgrade Pending</h4><div class="card-header-action"><a href="{{ route('admin.upgrades.index', ['status' => 'pending']) }}">Lihat semua</a></div></div><div class="card-body"><p>{{ $pendingUpgrades }} pengajuan menunggu keputusan.</p>@if($pendingUpgrades === 0)<div class="dashboard-empty-state"><div class="dashboard-empty-state__icon"><i data-feather="arrow-up-circle"></i></div><p class="lead">Tidak ada upgrade pending.</p></div>@else<a href="{{ route('admin.upgrades.index', ['status' => 'pending']) }}" class="btn btn-sm btn-primary">Buka pengajuan</a>@endif</div></div></div>
    </div>
