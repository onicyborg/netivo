<div class="main-sidebar sidebar-style-2"><aside id="sidebar-wrapper">
    ...
    <ul class="sidebar-menu">
        ...
        @if($role === 'admin')
            ...
            @foreach([['admin.services.*','admin.services.index','package','Layanan'],['admin.payment-methods.*','admin.payment-methods.index','credit-card','Metode Pembayaran'],['admin.customers.*','admin.customers.index','users','Customer'],['admin.bills.*','admin.bills.index','file-text','Tagihan'],['admin.payments.*','admin.payments.index','check-circle','Verifikasi Pembayaran'],['admin.settings.*','admin.settings.index','settings','Pengaturan'],['admin.cron-logs.*','admin.cron-logs.index','activity','Log Cron'],['admin.system-logs.*','admin.system-logs.index','shield','Audit Log'],['admin.reports.*','admin.reports.index','bar-chart-2','Laporan Harian'],['admin.upgrades.*','admin.upgrades.index','trending-up','Upgrade / Downgrade Layanan']] as [$pattern,$route,$icon,$label])<li class="{{ request()->routeIs($pattern) ? 'active' : '' }}"><a class="nav-link" href="{{ route($route) }}"><i data-feather="{{ $icon }}"></i><span>{{ $label }}</span></a></li>@endforeach
